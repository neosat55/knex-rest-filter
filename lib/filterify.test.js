...
describe('nested', () => {
  ...
  it('some test 1', () => {
    const filter = {
      $and: [
        { $or: [{ entityId: 1 }, { entityType: 2 }] },
        { $or: [{ entityId: 3 }, { entityType: 3 }] }
      ]
    };

    const want = `(("entityId" = 1 or "entityType" = 2) and ("entityId" = 3 or "entityType" = 3))`;
    helper(filter, want);
  });
  it('some test 2', () => {
    const filter = {
    ...
    const want = `("id" = 1 and ("entityId" = 3 or "entityType" = 3))`;
    helper(filter, want);
  });
  it('some test 3', () => {
    const filter = {
    ...
    const want = `("id" <= 3 and ("entityId" = 3 or "entityType" = 3))`;
    helper(filter, want);
  });
  it('some test 4', () => {
    const filter = {
    ...
    const want = `(("name" = 1 or "city" = 2) or ("entityId" = 1 and "entityType" = 2))`;
    helper(filter, want);
  });
  it('some test 5', () => {
    const filter = {
    ...
    const want = `(("name" = 1 and "city" = 2) and ("entityId" = 1 or "entityType" = 2))`;
    helper(filter, want);
  });
});
describe('plugins', () => {
  ...
  it('keep plugin should keep nested keys with nested $or', () => {
    const filter = {
      $and: [
        {
          $or: [
    ...
    const keepKeys = ['keep'];
    const want = `(("keep"."me" in (1, 2, 3) or "keep"."us" = 1))`;

    helper2([new Keeper(keepKeys)], filter, want);
  });
  it('keep plugin should keep nested dotted keys in all objects', () => {
    const filter = {
      $and: [
        { name: { $gt: 12 } },
        [{ hey: 1 }],
