...
describe('filterify', () => {
  ...
  it.only('should apply `or` statement inside column', () => {
...
describe('helpers', () => {
  const tests = [
    ...
    [
      `multi array case one grouping`,
      {$or: [[{name: 1}, {city: 2}]]},
      1
    ],
    [
      `multi array many grouping`,
      {$or: [[{name: 1}, {city: 2}], [{name: 1}, {city: 2}]]},
      2
    ],
    [
      `or with simple`,
  ...
  it.each(tests)('%s', (_, filter, want) => {
    const level = helpers.level(filter);
